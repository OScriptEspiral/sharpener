...
def fetch_access_token(code, github_config):
    ...
    github_response = requests.get(
        github_config["oauth_uri"], params=params, headers=headers
    )
...
def fetch_user_data(access_token, github_config):
    ...
    github_response = requests.get(
        github_config["user_uri"], headers=headers, params=params
    )
    fetched_data = github_response.json()
    if not fetched_data.get("email"):
        fetched_data["email"] = fetch_private_email(
            access_token, github_config
        )
...
def fetch_private_email(access_token, github_config):
    ...
    github_response = requests.get(
        email_resource_uri, headers=headers, params=params
    )
    fetched_data = github_response.json()
...
def create_users_blueprint(db_session, request, github_config):
    ...
    def authenticate_user(code):
        ...
        if not access_token:
            return Response(
                f"Something went wrong. Your github code\
                             was invalid.",
                status=400,
            )
        ...
        if existing_user:
            update_user_info(
                db_session, existing_user, user_data, access_token
            )
